...
public final class QuakeUtils {
    ...
    public static ArrayList<QuakeModel> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray features = root.getJSONArray(FEATURES_JSONARRAY);
            for (int i = 0;i <features.length();i++){
                JSONObject jsonObject =features.getJSONObject(i).getJSONObject(PROPERTIES);
                QuakeModel quakeModel = new QuakeModel();
                quakeModel.setQuakeMagnitude(jsonObject.getDouble(MAGNITUDE));
                quakeModel.setQuakeLocation(jsonObject.getString(LOCATION));
                quakeModel.setQuakeDate(jsonObject.getLong(DATE));
                quakeModel.setQuakeUrl(jsonObject.getString(URL));
                earthquakes.add(quakeModel);
            }
